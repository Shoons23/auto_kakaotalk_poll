...
def get_this_week_dates_excluding_saturday(reference_date=None):
    """
    reference_date(=datetime.date)를 기준으로,
    그 주 월요일(weekday=0)부터 일요일(weekday=6)까지
    월~금(weekday 0~4)과 일요일(weekday=6)만 [(datetime.date, '영어 요일 약자'), ...] 리스트로 반환.
    """
...
def format_english_date(day: datetime.date, weekday_name: str) -> str:
    """
    예: day=2025-05-26, weekday_name='Mon'
    → "250526 (Mon)"
    """
...
def load_coords(json_path="coords.json"):
    """
    coords.json 파일을 읽어서 COORD 딕셔너리로 반환.
    파일이 없거나 읽는 데 실패하면 프로그램을 종료.
    """
...
def click_and_type_for_title(x, y, text):
    """
    제목 입력란(x, y)을 클릭해서 포커스를 옮긴 뒤,
    1) 충분히 대기
    2) Ctrl+A 로 플레이스홀더 포함 기존 전체 선택
    3) 덮어쓰기
    4) 추가 대기
    """
    # 1) 제목란 클릭
...
def fill_poll(title: str, options: list[str], coords: dict):
    ...
    click_and_type_for_title(tx, ty, title)
...
# ---------------------------------------------
def main():
    # 1) coords.json 파일에서 좌표 불러오기
    coords = load_coords("coords.json")

    # 2) 이번 주 월~금 + 일요일 날짜 생성 (토요일 제외)
    week = get_this_week_dates_excluding_saturday()
    # YYYYMMDD 형식 + 영어 요일 약자로 변환
    ...
    try:
        ...
        print("✅ [TEST MODE] 투표 생성 시뮬레이션 완료. 마지막 단계 좌표를 확인했습니다.")
